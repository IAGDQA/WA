...
namespace NodeRED_WAConversionNodeTest
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        public void StartTest()
        {
            ...
            EventLog.AddLog("===NodeRED_WAConversionNode test start (by iATester)===");
            ...
            EventLog.AddLog("===NodeRED_WAConversionNode test end (by iATester)===");
        ...
        long Form1_Load(string sProjectName, string sWebAccessIP, string sTestLogFolder, string sBrowser)
        {
             ...
             * 6 刪除第2個分頁
             */
            bool bNodeREDResult = bNodeREDCoversionNodeTest(sProjectName);
        ...
        private bool bNodeREDCoversionNodeTest(string sProjectName)
        {
            bool bTestResult = true;
            string[] sConversionNodeName = { "StringToNumber", "NumberToString", "StringToBoolean", "NumberToBoolean", "BooleanToString", "BooleanToNumber" };
            string[] sOutputType = { "number", "string", "boolean", "boolean", "string", "number" };
            string[] sOutputValue = { "-4.5678", "-4.5678", "false", "false", "false", "1" };

            for (int i = 1; i <= sConversionNodeName.Length; i++)
            {
                //Step1: add workspace
                ...
                string sourceFile = sCurrentFilePath + string.Format("\\NodeREDSample\\{0}.txt", sConversionNodeName[i - 1]);
                ...
                Thread.Sleep(2000);
                api.ById("node-input-workspace-name").Enter(sConversionNodeName[i - 1]).Exe();
                Thread.Sleep(2000);
        ...
        {
            long lErrorCode = (long)ErrorCode.SUCCESS;
            EventLog.AddLog("===NodeRED_WAConversionNode test start===");
            CheckifIniFileChange();
            EventLog.AddLog("Project= " + ProjectName.Text);
            EventLog.AddLog("WebAccess IP address= " + WebAccessIP.Text);
            lErrorCode = Form1_Load(ProjectName.Text, WebAccessIP.Text, TestLogFolder.Text, Browser.Text);
            EventLog.AddLog("===NodeRED_WAConversionNode test end===");
        }
