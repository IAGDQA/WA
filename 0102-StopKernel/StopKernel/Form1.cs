...
namespace StopKernel
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        public void StartTest()
        {
            ...
            EventLog.AddLog("===Stop Kernel start (by iATester)===");
            ...
            EventLog.AddLog("===Stop Kernel end (by iATester)===");

            Thread.Sleep(120000);   // stop kernel for 120s for Redundancy test to check "Recover to Primary" function (only in iAtester test)
        ...
        private void StopNode(IAdvSeleniumAPI api, string sTestLogFolder)
        {
            ...
            PrintScreen("Stop Node result", sTestLogFolder);
            api.Close();
        ...
        {
            long lErrorCode = (long)ErrorCode.SUCCESS;
            EventLog.AddLog("===Stop Kernel start===");
            CheckifIniFileChange();
            EventLog.AddLog("Project= " + ProjectName.Text);
            EventLog.AddLog("WebAccess IP address= " + WebAccessIP.Text);
            lErrorCode = Form1_Load(ProjectName.Text, WebAccessIP.Text, TestLogFolder.Text, Browser.Text);
            EventLog.AddLog("===Stop Kernel end===");
        }
