...
namespace CreateSystemTags
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        string baseUrl;
        string sTestItemName = "CreateSystemTags";
        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            if (bPartResult == true)
            {
                EventLog.AddLog("Create system tags");
                sw.Reset(); sw.Start();
                try
                {
                    driver.SwitchTo().Frame("leftFrame");
                    if (wcf.IsTestElementPresent(driver, "XPath", "//a[contains(@href, '/broadWeb/bwMainRight.asp?pos=SysList') and contains(@href, 'name=TestSCADA')]"))
                    {
                        driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/bwMainRight.asp?pos=SysList') and contains(@href, 'name=TestSCADA')]")).Click();
                        Thread.Sleep(1500);
                        EventLog.AddLog("Delete the system tag");
                    ...
                    EventLog.AddLog("Create System Tags...");
                    CreateSystemTag();
                    //PrintStep("Create System Tags");
                }
                catch (Exception ex)
                {
                    EventLog.AddLog(@"Error occurred Create system tags: " + ex.ToString());
                    bPartResult = false;
                }
                sw.Stop();
                PrintStep("Create", "Create system tags", bPartResult, "None", sw.Elapsed.TotalMilliseconds.ToString());
        ...
        private void CreateSystemTag()
        {
            ...
            driver.SwitchTo().Frame("rightFrame");
            //driver.FindElement(By.XPath("//a[7]/font/b")).Click();
            driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/bwMainRight.asp?pos=SysList')]")).Click();
            driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/tag/tagPg.asp') and contains(@href, 'dt=SysType')]")).Click();
            Thread.Sleep(1000);
            new SelectElement(driver.FindElement(By.Name("ParaName"))).SelectByText("TM_Sec");
            Thread.Sleep(500);
            ...
            for (int i = 1; i <= 5; i++)
            {
                try
                {
                    ...
                    driver.FindElement(By.Name("TagName")).SendKeys("SystemSec_" + i.ToString("0000"));
        ...
        private void SetupBasicSystemTagConfig()
        {
