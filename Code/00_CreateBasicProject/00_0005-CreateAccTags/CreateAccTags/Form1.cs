...
namespace CreateAccTags
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        string baseUrl;
        string sTestItemName = "CreateAccTags";
        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            //Create Acc Tags test
            if (bPartResult == true)
            {
                EventLog.AddLog("Create Acc Tags");
                sw.Reset(); sw.Start();
                try
                {
                    driver.SwitchTo().Frame("leftFrame");
                    if (wcf.IsTestElementPresent(driver, "XPath", "//a[contains(@href, '/broadWeb/bwMainRight.asp?pos=AccList') and contains(@href, 'name=TestSCADA')]"))
                    {
                        driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/bwMainRight.asp?pos=AccList') and contains(@href, 'name=TestSCADA')]")).Click();
                        Thread.Sleep(1500);
                        EventLog.AddLog("Delete the Acc tag");
                ...
                {
                    EventLog.AddLog(@"Error occurred create Acc Tags: " + ex.ToString());
                    bPartResult = false;
                }
                sw.Stop();
                PrintStep("Create", "Create Acc Tags", bPartResult, "None", sw.Elapsed.TotalMilliseconds.ToString());
        ...
        {
            driver.SwitchTo().Frame("rightFrame");
            //driver.FindElement(By.XPath("//a[4]/font/b")).Click();
            driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/bwMainRight.asp?pos=AccList')]")).Click();
            driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/tag/tagPg.asp') and contains(@href, 'dt=AccType')]")).Click();

            // Acc point
            EventLog.AddLog("Create  accumulation point");
            driver.FindElement(By.Name("AccFreq")).Clear();
            driver.FindElement(By.Name("AccFreq")).SendKeys("5");
            ...
            for (int i = 1; i <= 5; i++)
            {
                try
                {
                    ...
                    driver.FindElement(By.Name("TagName")).SendKeys("Acc_" + i.ToString("0000"));
                    driver.FindElement(By.Name("Source")).Clear();
                    driver.FindElement(By.Name("Source")).SendKeys("ConDis_" + i.ToString("0000"));
