...
namespace CreateModbusDeviceTags
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        public void StartTest()
        {
            ...
            EventLog.AddLog("===Create Modebus Device Tags end (by iATester)===");
        ...
        long Form1_Load(string sProjectName, string sWebAccessIP, string sTestLogFolder, string sBrowser)
        {
            ...
            AddDevice(sWebAccessIP);
            PrintStep("Add Device");

            ReturnSCADAPage();
            PrintStep("Return SCADA Page");
        ...
        private void AddDevice(string sWebAccessIP)
        {
            ...
            api.ByXpath("//a[contains(@href, '/broadWeb/bwMainRight.asp?pos=comport') and contains(@href, 'name=3')]").Click(); // port = 3
            api.SwitchToCurWindow(0);
            api.SwitchToFrame("rightFrame", 0);
            api.ByXpath("//a[3]/font/b").Click();
            api.ByName("DeviceName").Clear();
            api.ByName("DeviceName").Enter("ModSim").Exe();
            api.ByName("DeviceTypeSel").SelectTxt("Modicon").Exe();
            api.ByName("P_IPAddr").Clear();
            api.ByName("P_IPAddr").Enter(sWebAccessIP).Exe();
            api.ByName("P_PortNbr").Clear();
            api.ByName("P_PortNbr").Enter("502").Exe();
            api.ByName("P_DevAddr").Clear();
            api.ByName("P_DevAddr").Enter("1").Submit().Exe();
        ...
        private void CreateModbusTag()
        {
            ...
            if (lErrorCode == 0)
            {
                api.SwitchToCurWindow(0);
                api.SwitchToFrame("rightFrame", 0);
                api.ByXpath("//a[2]/font/b").ClickAndWait(1000);
            }
            PrintStep("Create Modbus Tag");

            // Create AI tag
            EventLog.AddLog("Create AI tags start...");
            CreateModbusAITags();
            PrintStep("Create AI tags");

            // Create AO tag
            EventLog.AddLog("Create AO tags start...");
            CreateModbusAOTags();
            PrintStep("Create AO tags");

            // Create DI tag
            EventLog.AddLog("Create DI tags start...");
            CreateModbusDITags();
            PrintStep("Create DI tags");

            // Create DO tag
            EventLog.AddLog("Create DO tags start...");
            CreateModbusDOTags();
            PrintStep("Create DO tags");
        ...
        {
            api.ByName("ParaName").SelectVal("AI").Exe();
            Thread.Sleep(1500);
            ...
            for (int i = 1; i <= 5; i++)
            {
                try
                {
                    ...
                    api.ByName("TagName").Enter("AT_AI0" + i.ToString("000")).Submit().Exe();
                }
                catch (Exception ex)
                {
        ...
        {
            api.ByName("ParaName").SelectVal("AO").Exe();
            Thread.Sleep(1500);
            SetupBasicAnalogTagConfig();
            for (int i = 1; i <= 5; i++)
            {
                ...
                {
                    api.ByName("TagName").Clear();
                    api.ByName("TagName").Enter("AT_AO0" + i.ToString("000")).Submit().Exe();
                }
                catch (Exception ex)
                {
        ...
        {
            api.ByName("ParaName").SelectVal("DI").Exe();
            Thread.Sleep(1500);
            ...
            for (int i = 1; i <= 5; i++)
            {
                ...
                {
                    api.ByName("TagName").Clear();
                    api.ByName("TagName").Enter("AT_DI0" + i.ToString("000")).Submit().Exe();
                }
                catch (Exception ex)
                {
        ...
        {
            api.ByName("ParaName").SelectVal("DO").Exe();
            Thread.Sleep(1500);
            SetupBasicDigitalTagConfig();
            for (int i = 1; i <= 5; i++)
            {
                ...
                {
                    api.ByName("TagName").Clear();
                    api.ByName("TagName").Enter("AT_DO0" + i.ToString("000")).Submit().Exe();
                }
                catch (Exception ex)
                {
        ...
        private void Start_Click(object sender, EventArgs e)
        {
            ...
            EventLog.AddLog("WebAccess IP address= " + WebAccessIP.Text);
            lErrorCode = Form1_Load(ProjectName.Text, WebAccessIP.Text, TestLogFolder.Text, Browser.Text);
            EventLog.AddLog("===Create Modbus Device Tags end===");
        }
