        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            if (bPartResult == true)
            {
                try
                {
                    ...
                    EventLog.AddLog("Create DataTransferData");
        ...
        private void CheckifIniFileChange()
    {
        ...
        private void CheckData()
        {
            try
            {
                ...
                ReadOnlyCollection<IWebElement> links = driver.FindElements(By.XPath("//a[contains(@href, 'deldatatrans.asp')]"));

                for (int i = 0; i < links.Count; i++)
                {
                    driver.FindElement(By.XPath("/html/body/table/tbody/tr[3]/td/center/table/tbody/tr[2]/td[6]/font/a")).Click();
                    Thread.Sleep(1000);
