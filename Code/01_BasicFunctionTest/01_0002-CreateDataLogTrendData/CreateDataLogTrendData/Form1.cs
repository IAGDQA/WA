...
namespace CreateDataLogTrendData
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        string baseUrl;
        string sTestItemName = "CreateDataLogTrendData";
        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            if (bPartResult == true)
            {
                ...
                try
                {
                    ...
                    driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/bwMainRight.asp') and contains(@href, 'pos=DLogList')]")).Click();
                    if (wcf.IsTestElementPresent(driver, "XPath", "//a[contains(@href, '/broadWeb/datalog/deldatalog.asp') and contains(@href, 'GroupNbr=1')]"))
                    {
                        EventLog.AddLog("Delete the group 1 datalog trend");
                        driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/datalog/deldatalog.asp') and contains(@href, 'GroupNbr=1')]")).Click();
                        Thread.Sleep(1000);
                        driver.SwitchTo().Alert().Accept();
                        Thread.Sleep(1000);
                    }

                    EventLog.AddLog("Copy datalog templete to webaccess folder");
                    {
                        ...
                        string destFile1_1 = string.Format("C:\\WebAccess\\Node\\config\\{0}_TestSCADA\\kst\\DatalogTrendHotKey.kmp", sPrimaryProject);
                        ...
                        string destFile2_1 = string.Format("C:\\WebAccess\\Node\\config\\{0}_TestSCADA\\bgr\\dlogtrd.bxx", sPrimaryProject);
                        ...
                        string destFile3_1 = string.Format("C:\\WebAccess\\Node\\config\\{0}_TestSCADA\\drw\\dlogtrd.dxx", sPrimaryProject);
                    ...
                    EventLog.AddLog("Create the group 1 datalog trend");
                    CreateDataLogTrend();
                }
                catch (Exception ex)
                {
                    EventLog.AddLog(@"Error occurred Create Data Log Trend: " + ex.ToString());
                    bPartResult = false;
                }
                sw.Stop();
                PrintStep("Create", "Create Data Log Trend", bPartResult, "None", sw.Elapsed.TotalMilliseconds.ToString());
        ...
        private void CreateDataLogTrend()
        {
            //driver.SwitchTo().Frame("rightFrame");
            //driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/bwMainRight.asp') and contains(@href, 'pos=DLogList')]")).Click();
            driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/datalog/datalogPg.asp') and contains(@href, 'action=add_datalog')]")).Click();
            ...
            for (int i = 0; i < CalcTagName.Length; i++)
            {
                try
                {
                    ...
                    driver.FindElement(By.Name(tagName)).SendKeys("Calc_" + CalcTagName[i]);
                    driver.FindElement(By.Name(dispH)).SendKeys("100");
                    driver.FindElement(By.Name(dispL)).SendKeys("0");
                }
                catch (Exception ex)
                {
