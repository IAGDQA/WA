    ...
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            */
            if (bPartResult == true)
            {
                EventLog.AddLog("AnalogChangeLog test");
                sw.Reset(); sw.Start();
                try
                {
                    bool bAnalogChangeLogChk = AnalogChangeLogDataCheck(sPrimaryProject);
                }
                catch (Exception ex)
                {
                    EventLog.AddLog(@"Error occurred AnalogChangeLog test: " + ex.ToString());
                    bPartResult = false;
                }
                sw.Stop();
                PrintStep("Verify", "AnalogChangeLog test", bPartResult, "None", sw.Elapsed.TotalMilliseconds.ToString());
