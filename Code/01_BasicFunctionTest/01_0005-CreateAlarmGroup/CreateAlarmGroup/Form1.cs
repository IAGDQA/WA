...
namespace CreateAlarmGroup
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        string baseUrl;
        string sTestItemName = "CreateAlarmGroup";
        ...
        private void CreateAlarmGroup()
        {
            try
            {
                ...
                //driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/bwMainRight.asp') and contains(@href, 'pos=AlmGrpList')]")).Click();
                driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/AlarmGrp/AlmGrpPg.asp') and contains(@href, 'action=add_almgr')]")).Click();
                ...
                for (int i = 0; i < AlarmTagName.Length; i++)
                {
                    ...
                    driver.FindElement(By.Name(tagName)).SendKeys(AlarmTagName[i]);
                }
                driver.FindElement(By.Name("TagName")).SendKeys("Alarm Group Set");
                driver.FindElement(By.Name("Submit")).Click();
            }
            catch (Exception ex)
            {
                EventLog.AddLog("CreateAlarmGroup error: " + ex.ToString());
                bPartResult = false;
            }
        }
        ...
        private void CheckData()
        {
            try
            {
                ...
                ReadOnlyCollection<IWebElement> links = driver.FindElements(By.XPath("//a[contains(@href, 'delalmgrp.asp')]"));

                for (int i = 0; i < links.Count; i++)
                {
                    driver.FindElement(By.XPath("/html/body/table/tbody/tr[3]/td/center/table/tbody/tr[2]/td[4]/font/a")).Click();
                    Thread.Sleep(1000);
