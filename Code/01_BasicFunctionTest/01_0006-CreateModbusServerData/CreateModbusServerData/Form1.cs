...
namespace CreateModbusServerData
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            if (bPartResult == true)
            {
                try
                {
                    ...
                    if (bPartResult == true)
                        CreateModbusServerConfig();
                }
                catch (Exception ex)
                {
                    EventLog.AddLog("Create	ModbusServerConfig error: " + ex.ToString());
                    bPartResult = false;
                }
                Thread.Sleep(1000);
            }
        ...
        private void CreateModbusServerConfig()
        {
            try
            {
                EventLog.AddLog("Create	ModbusServerConfig...");
                //driver.SwitchTo().Frame("rightFrame");
                //driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/modbusSrv/ModbusSrvPortLst.asp')]")).Click();
                driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/modbusSrv/addupModbusSrvPort.asp') and contains(@href, 'act=add')]")).Click();

                driver.FindElement(By.Name("srvName")).SendKeys("ModbusServer");
                new SelectElement(driver.FindElement(By.Id("PortType"))).SelectByText("TCPIP");
                driver.FindElement(By.Name("ENABLE_EXCEPTION")).Click();
                driver.FindElement(By.XPath("//button[@type='submit']")).Click();
                driver.FindElement(By.XPath("//a[contains(@href, '/broadWeb/modbusSrv/addModbusSrvTag.asp')]")).Click();

                for (int i = 1; i <= 10; i++)
                {
                    driver.FindElement(By.Name("Address")).Clear();
                    driver.FindElement(By.Name("Address")).SendKeys("4" + i.ToString("0000"));
            ...
            {
                EventLog.AddLog("CreateModbusServerConfig error: " + ex.ToString());
                bPartResult = false;
            }
        }
        ...
        private void CheckData()
        {
            try
            {
                ...
                ReadOnlyCollection<IWebElement> links = driver.FindElements(By.XPath("//a[contains(@href, 'ModbusSrvPortDel.asp')]"));

                for (int i = 0; i < links.Count; i++)
                {
                    driver.FindElement(By.XPath("/html/body/table/tbody/tr[3]/td/center/table/tbody/tr[3]/td[6]/font/a")).Click();
                    Thread.Sleep(1000);
