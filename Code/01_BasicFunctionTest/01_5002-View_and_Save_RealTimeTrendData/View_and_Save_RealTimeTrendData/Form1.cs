...
namespace View_and_Save_RealTimeTrendData
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        string baseUrl;
        string sTestItemName = "View_and_Save_RealTimeTrendData";
        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            //View Real Time Trend
            if (bPartResult == true)
            {
                EventLog.AddLog("View Real Time Trend");
                sw.Reset(); sw.Start();
                try
                {
                    EventLog.AddLog("Start real time log trend");
                    StartViewRealTimeTrend(sLanguage);
                }
                catch (Exception ex)
                {
                    EventLog.AddLog(@"Error occurred View Real Time Trend: " + ex.ToString());
                    bPartResult = false;
                }
                sw.Stop();
                PrintStep("View", "View Real Time Trend", bPartResult, "None", sw.Elapsed.TotalMilliseconds.ToString());

                Thread.Sleep(1000);
            }

            EventLog.AddLog("Start wait 180s...");
            Thread.Sleep(180000);   // wait 180s for data output
        ...
        private void StartViewRealTimeTrend(string slanguage)
        {
            // Start view
