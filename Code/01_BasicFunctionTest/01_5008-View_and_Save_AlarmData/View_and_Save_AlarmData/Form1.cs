...
namespace View_and_Save_AlarmData
{
    public partial class Form1 : Form, iATester.iCom
    {
        ...
        string baseUrl;
        string sTestItemName = "View_and_Save_AlarmData";
        ...
        long Form1_Load(string sPrimaryProject, string sPrimaryIP, string sSecondaryProject, string sSecondaryIP, string sTestLogFolder, string sBrowser, string sUserEmail, string sLanguage)
        {
            ...
            if (bPartResult == true)
            {
                EventLog.AddLog("Alarm data test");
                sw.Reset(); sw.Start();
                try
                {
                    EventLog.AddLog("Check Alarm data...");
                    ViewAlarmData(sTestLogFolder, sLanguage);
                }
                catch (Exception ex)
                {
                    EventLog.AddLog(@"Error occurred Alarm data test: " + ex.ToString());
                    bPartResult = false;
                }
                sw.Stop();
                PrintStep("Verify", "Alarm data test", bPartResult, "None", sw.Elapsed.TotalMilliseconds.ToString());
